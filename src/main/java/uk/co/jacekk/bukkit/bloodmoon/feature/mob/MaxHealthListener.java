...
public class MaxHealthListener implements Listener {
	...
	public void onCreatureSpawn(CreatureSpawnEvent event) {
		...
		if (plugin.isActive(world) && plugin.isFeatureEnabled(world, Feature.MAX_HEALTH)
				&& worldConfig.getStringList(Config.FEATURE_MAX_HEALTH_MOBS).contains(entity.getType().name())) {
			doEntity(entity, worldConfig);
		}
	}
