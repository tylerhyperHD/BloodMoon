...
public abstract class BloodMoonEntityMonster extends BloodMoonEntityLiving {
	...
	protected Block getBreakableTargetBlock() {
		Location direction = nmsEntity.getGoalTarget().getBukkitEntity().getLocation()
				.subtract(bukkitEntity.getLocation());
		...
		return nmsEntity.world.getWorld().getBlockAt((int) Math.floor(nmsEntity.locX + bdx),
				(int) Math.floor(nmsEntity.locY), (int) Math.floor(nmsEntity.locZ + bdz));
	}
	...
	protected void attemptBreakBlock(PluginConfig worldConfig, Block block) {
		...
		if (type != Material.AIR
				&& worldConfig.getStringList(Config.FEATURE_BREAK_BLOCKS_BLOCKS).contains(type.name())) {
			Location location = block.getLocation();

			if (this.rand.nextInt(100) < 80) {
				if (this.rand.nextInt(100) < 50) {
					nmsEntity.world.getWorld().playSound(location, Sound.ENTITY_ZOMBIE_ATTACK_WOODEN_DOOR,
							Math.min(this.rand.nextFloat() + 0.2f, 1.0f), 1.0f);
				}
			} else {
				...
				if (!event.isCancelled()) {
					...
					if (worldConfig.getBoolean(Config.FEATURE_BREAK_BLOCKS_REALISTIC_DROP)) {
						...
						if (worldConfig.getBoolean(Config.FEATURE_BREAK_BLOCKS_DROP_ITEMS)) {
							nmsEntity.world.getWorld().dropItemNaturally(location,
									new ItemStack(type, 1, block.getData()));
						}
					}
				}
			}
