...
public final class BloodMoon extends BasePlugin {
			...
			try {
				...
				if (listener != null) {
					getServer().getPluginManager()
							.registerEvents(listener.getConstructor(BloodMoon.class).newInstance(this), this);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	/**
	 * Sets up the config for a world. This should only be used by other plugins if
	 * a world is being loaded that would not call a WorldInitEvent.
	 *
	...
	public PluginConfig createConfig(World world) {
		...
		if (!worldConfig.containsKey(worldName)) {
			PluginConfig newConfig = new PluginConfig(new File(getBaseDirPath() + File.separator + worldName + ".yml"),
					Config.class, getLogger());
